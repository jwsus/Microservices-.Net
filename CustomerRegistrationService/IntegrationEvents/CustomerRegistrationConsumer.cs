...
public class CustomerRegistrationConsumer : BackgroundService
{
    private readonly IConnection _connection;
    private readonly IModel _channel;
    private readonly IEventHandler<CustomerRegisteredEvent> _eventHandler;
    private readonly RabbitMQSettings _settings;

    public CustomerRegistrationConsumer(IEventHandler<CustomerRegisteredEvent> eventHandler,  IOptions<RabbitMQSettings> settings)
    {
    ...
    public async void ProcessMessage(string message, BasicDeliverEventArgs  ea)
    {
        try
        {
            var @event = JsonConvert.DeserializeObject<CustomerRegisteredEvent>(message);
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (model, ea) =>
        {
            ...
            try
            {
                ...
                var teste = JsonConvert.DeserializeObject<CustomerRegisteredEvent>(content);

                var responseEvent = new CustomerCreatedIntegrationEvent { CustomerId = teste.CustomerId, CorrelationId = ea.BasicProperties.CorrelationId };
                var responseMessage = JsonConvert.SerializeObject(responseEvent);
                var responseBody = Encoding.UTF8.GetBytes(responseMessage);

                var properties = _channel.CreateBasicProperties();
                properties.CorrelationId = ea.BasicProperties.CorrelationId;

                _channel.BasicPublish(exchange: "", routingKey: ea.BasicProperties.ReplyTo, basicProperties: properties, body: responseBody);
        ...
        _channel.BasicConsume("CustomerRegisteredQueue", false, consumer);
    ...
    private void DeclareQueue()
    {
        ...
        var args = new Dictionary<string, object>
        {
            ...
            {"x-dead-letter-routing-key", "CustomerRegisteredQueueDlq"},
            {"x-message-ttl", 60000},  
            {"x-max-length", 10000}, 
            {"x-max-delivery-count", 5}
        };

        _channel.QueueDeclare(queue: "CustomerRegisteredQueue",
                            durable: true,
                            exclusive: false,
                            autoDelete: false,
                            arguments: args);

        _channel.QueueDeclare(queue: "CustomerRegisteredQueueDlq",
                            durable: true,
                            exclusive: false,
                            autoDelete: false,
                            arguments: null);
